...
UINT32	SaveLoadScreenHandle()
{
	...
	if( gfSaveLoadScreenEntry )
	{
		...
		//save the new rect
		BlitBufferToBuffer(guiRENDERBUFFER, guiSAVEBUFFER, 0, 0, SCREEN_BUFFER_WIDTH, SCREEN_BUFFER_HEIGHT-40 );
	}
...
*/
	...
	MSYS_DefineRegion( &gSLSEntireScreenRegion, 0, 0, SCREEN_BUFFER_WIDTH, SCREEN_BUFFER_HEIGHT, MSYS_PRIORITY_HIGH-10,
...
//			SaveGameHeader.bSectorZ = gbWorldSectorZ;
		...
		if( !gfSaveGame && gfKeyState[ CTRL ] && gbSelectedSaveLocation == bEntryID )
		{
			...
			CHAR16		zCampaignString[256];
			
			//Create a string for difficulty level
			swprintf( zDifString, L"%s", gzGIOScreenText[ GIO_EASY_TEXT + SaveGameHeader.sInitialGameOptions.ubDifficultyLevel - 1 ] );

			if ( stricmp( SaveGameHeader.zCampaignName, "Unfinished Business" ) == 0 )
			{
				swprintf( zCampaignString, zNewTacticalMessages[ TCTL_MSG__DEFAULT_CAMPAIGN_LABEL ], SaveGameHeader.zCampaignName );
			}
			else
			{
				swprintf( zCampaignString, zNewTacticalMessages[ TCTL_MSG__CAMPAIGN_LABEL ], SaveGameHeader.zCampaignName );
			}

			//make a string containing the extended options
			swprintf( zMouseHelpTextString, L"%s, %s, %s, %s", zDifString,
