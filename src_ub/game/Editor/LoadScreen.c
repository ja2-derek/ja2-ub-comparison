...
extern INT8		gbInsertionCode;
extern INT16	gsInsertionGridNo;
...
void LoadSaveScreenEntry()
{
	...
	// ATE: Make sure directory exists!
	{
		CHAR8 zTempDir[512];
		CHAR8 zExeDir[500];
		...
		sprintf( zTempDir, "%s\\Data\\Campaigns\\%s", zExeDir, GetCurrentCampaign( ) );
		CreateDirectory( zTempDir, NULL );
	}

	sprintf( zFileSpec, "%s\\*.dat", GetMapsDirectory( ) );

	iTopFileShown = iTotalFiles = 0;
	if( GetFileFirst( zFileSpec, &FileInfo) )
	{
...
UINT32 LoadSaveScreenHandle(void)
{
	...
	switch( iFDlgState )
	{
		...
		case DIALOG_DELETE:
			...
			sprintf( gszCurrFilename, "%s\\%S", GetMapsDirectory( ), gzFilename );
...
void UpdateWorldInfoCallback( GUI_BUTTON *b, INT32 reason )
{
	if( reason & MSYS_CALLBACK_REASON_LBUTTON_UP )
		gfCreateRadarMap = b->uiFlags & BUTTON_CLICKED_ON ? TRUE : FALSE;
}
...
void SetGlobalSectorValues( UINT16 *szFilename )
{
	...
	if( ValidCoordinate() )
	{
		...
		if( gzFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] >= 'A' && gzFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] <= 'P' )
			gWorldSectorY = gzFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] - 'A' + 1;
		else
			gWorldSectorY = gzFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] - 'a' + 1;
		if( gzFilename[1+JA25_EXP_MAP_NAME_SIZE_OFFSET] == '1' && gzFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] >= '0' && gzFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] <= '6' )
			gWorldSectorX = ( gzFilename[1+JA25_EXP_MAP_NAME_SIZE_OFFSET] - 0x30 ) * 10 + ( gzFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] - 0x30 );
		else
			gWorldSectorX = ( gzFilename[1+JA25_EXP_MAP_NAME_SIZE_OFFSET] - 0x30 );
	...
	switch( gbCurrentFileIOStatus )
	{
		...
		case SAVING_MAP: //save map
			...
			if( gfShowPits )
				RemoveAllPits();
			OptimizeSchedules();
			if ( !SaveWorld( ubNewFilename, gfCreateRadarMap ) )
			{
				if( gfErrorCatch )
				{
		...
		case LOADING_MAP: //load map
			...
			if( !LoadWorld( ubNewFilename, FALSE, TRUE ) )
			{ //Want to override crash, so user can do something else.
				EnableUndo();
				SetPendingNewScreen( EDIT_SCREEN );
				gbCurrentFileIOStatus = IOSTATUS_NONE;
				gfGlobalError = FALSE;
				gfLoadError = TRUE;
				//RemoveButton( iTempButton );
				ScreenMsg( FONT_MCOLOR_RED, MSG_INTERFACE, gzEditorStrings[EDITOR_STR_ERROR_LOADING] );
			...
			if ( gMapInformation.ubLoadingScreenID == 0 )
			{
			...
			{
				gsInsertionGridNo = gMapInformation.sInsertionGridNo;
				gbInsertionCode = gMapInformation.ubInsertionDirection;

				if ( gMapInformation.fAutoDirection )
				{
...
BOOLEAN ExtractFilenameFromFields()
{
	...
	// ATE: see if we have an extension, if not, add .dat
	pDest = wcsstr( gzFilename, L"." );

	if( !pDest )
	{
		// Add extension
		wcscat( gzFilename, gzEditorStrings[EDITOR_STR_DAT_STR] );
	}
...
{
	if( gzFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] >= 'A' && gzFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] <= 'P' ||
		gzFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] >= 'a' && gzFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] <='p' )
	{
		UINT16 usTotal;
		if( gzFilename[1+JA25_EXP_MAP_NAME_SIZE_OFFSET] == '1' && gzFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] >= '0' && gzFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] <= '6' )
		{
			usTotal = ( gzFilename[1+JA25_EXP_MAP_NAME_SIZE_OFFSET] - 0x30 ) * 10 + ( gzFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] - 0x30 );
		}
		else if( gzFilename[1+JA25_EXP_MAP_NAME_SIZE_OFFSET] >= '1' && gzFilename[1+JA25_EXP_MAP_NAME_SIZE_OFFSET] <= '9')
		{
			if( gzFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] < '0' || gzFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] > '9' )
			{
				usTotal = ( gzFilename[1+JA25_EXP_MAP_NAME_SIZE_OFFSET] - 0x30 );
			}
			else
			{
				return FALSE;
...
BOOLEAN ValidFilename()
{
	...
	if( gzFilename[0] != '\0' );
	{
		if ( wcsicmp( gzFilename, L"H7.DAT" ) == 0 )
		{
			return( FALSE );
		}
		...
		if( !pDest )
			pDest = wcsstr( gzFilename, gzEditorStrings[EDITOR_STR_DAT_STR] );
//		if( pDest && pDest != gzFilename && pDest[4+JA25_EXP_MAP_NAME_SIZE_OFFSET] == '\0' )
