...
void CreateSummaryWindow()
{
	...
	iSummaryButton[ SUMMARY_BACKGROUND ] =
		CreateTextButton( 0, 0, 0, 0, BUTTON_USE_DEFAULT, 0, 0, SCREEN_BUFFER_WIDTH, 360, BUTTON_TOGGLE, MSYS_PRIORITY_HIGH - 1,
					...
					if( gpWorldItemsSummaryArray[ i ].o.usItem == index )
					{
								...
								gubSummaryItemMode == ITEMMODE_REAL && !(gpWorldItemsSummaryArray[ i ].usFlags & WORLD_ITEM_SCIFI_ONLY) )
						{
							...
							if( !pItem->bFrequency )
								...
								bFreqIndex = 2;
							else if( pItem->bFrequency == PANIC_FREQUENCY_4 )
								bFreqIndex = 3;
							else if( pItem->bFrequency == PANIC_FREQUENCY_5 )
								bFreqIndex = 4;
							else if( pItem->bFrequency == PANIC_FREQUENCY_6 )
								bFreqIndex = 5;
								...
								bFreqIndex = 9;
							else if( pItem->bFrequency == FIRST_MAP_PLACED_FREQUENCY + 5 )
								bFreqIndex = 10;
							else if( pItem->bFrequency == FIRST_MAP_PLACED_FREQUENCY + 6 )
								bFreqIndex = 11;
							else
								continue;
							if( index == SWITCH )
		...
		for( i = 0; i < 11; i++ )
		{
			if( uiTriggerQuantity[i] || uiActionQuantity[i] )
			{
				...
				switch( i )
				{
					...
					case 3:	swprintf( str, L"Panic4" );		break;
					case 4:	swprintf( str, L"Panic5" );		break;
					case 5:	swprintf( str, L"Panic6" );		break;
					case 6:	swprintf( str, L"Norm1" );		break;
					case 7:	swprintf( str, L"Norm2" );		break;
					case 8:	swprintf( str, L"Norm3" );		break;
					case 9:	swprintf( str, L"Norm4" );		break;
					case 10:	swprintf( str, L"Pressure Actions" );		break;
				}
				if( i < 10 )
				{
...
void RenderSummaryWindow()
{
	...
	if( gfRenderSummary )
	{
		...
		InvalidateRegion( 0, 0, SCREEN_BUFFER_WIDTH, 360 );
...

		{
			...
			INT32						iTotal = 0;
			INT32						iNumAdim = 0, iNumElite = 0, iNumArmy = 0, iNumBloodcat = 0;
			BASIC_SOLDIERCREATE_STRUCT *pBasic;

			SetFontForeground( FONT_YELLOW );
			mprintf( 10, 20, L"%s",  gzEditorStrings[ EDITOR_STR_PLACEMENTS ] );
			mprintf( 50, 30, L"%s:", gzEditorStrings[ EDITOR_STR_ADMINS ] );
			mprintf( 50, 40, L"%s:", gzEditorStrings[ EDITOR_STR_TROOPS ] );			
			mprintf( 50, 50, L"%s:", gzEditorStrings[ EDITOR_STR_ELITES ] );
			mprintf( 10, 60, L"%s", gzEditorStrings[ EDITOR_STR_TOTAL ] );

			pNode = gSoldierInitHead;
			while( pNode )
			{
				pBasic = pNode->pBasicPlacement;

				switch( pBasic->ubSoldierClass )
				{
			...
			mprintf( 100, 30, L"%d", iNumAdim);
			mprintf( 100, 40, L"%d", iNumArmy);			
			mprintf( 100, 50, L"%d", iNumElite);

			iTotal = ( iNumAdim + iNumArmy + iNumElite );

			mprintf( 100, 60, L"%d", iTotal );

			if ( iTotal < 32 )
			{
				SetFontForeground( FONT_RED );				
				mprintf( 100, 70, gzEditorStrings[ EDITOR_STR_MISSING ], ( 32 - iTotal ) );
			}

			if ( iNumBloodcat > 0 )
			{
				SetFontForeground( FONT_YELLOW );
				mprintf( 10, 90, L"%s", gzEditorStrings[ EDITOR_STR_NUM_BLOODCATS ] );
				mprintf( 100, 90, L"%d", iNumBloodcat );
			}
		}
...
//				DisableButton( iSummaryButton[ SUMMARY_LOAD ] );
			...
			else
			{
				...
				swprintf( str, L"%s%c%d", JA25_EXP_MAP_NAME_PREFIX, y + 'A', x + 1 );
...
//					DisableButton( iSummaryButton[ SUMMARY_LOAD ] );
	...
	if( gfWorldLoaded && !gfTempFile && gsSectorX )
	{
		...
		RectangleDraw( TRUE, x, y, x+11, y+11, Get16BPPColor( FROMRGB( 200, 200, 50 ) ), pDestBuf );
	}
	UnLockVideoSurface( FRAME_BUFFER );
...
void UpdateSectorSummary( UINT16 *gszFilename, BOOLEAN fUpdate )
{
	...
	gfRenderSummary = TRUE;
	//Extract the sector
	if( gszFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] < L'0' || gszFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] > L'9' )
		x = gszFilename[1+JA25_EXP_MAP_NAME_SIZE_OFFSET] - L'0';
	else
		x = (gszFilename[1+JA25_EXP_MAP_NAME_SIZE_OFFSET] - L'0') * 10 + gszFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] - L'0';
	if( gszFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] >= 'a' )
		y = gszFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] - L'a' + 1;
	else
		y = gszFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] - L'A' + 1;
	...
	if( fUpdate )
	{
		...
		if( gsSectorX > 9 )
			szCoord[3+JA25_EXP_MAP_NAME_SIZE_OFFSET] = '\0';
		else
			szCoord[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] = '\0';
...
void LoadGlobalSummary()
{
  ...
  INT32 x,y;
	...
	for( y = 0; y < 16; y++ )
	{
		for( x = 0; x < 16; x++ )
		{
			if ( SectorInfo[ ( SECTOR( y , x ) ) ].fCampaignSector )
			{
				continue;
			}

			gbSectorLevels[x][y] = 0;
			sprintf( szSector, "%S%c%d", JA25_EXP_MAP_NAME_PREFIX, 'A' + y, x + 1 );

			//main ground level
			sprintf( szFilename, "%S%c%d.dat", JA25_EXP_MAP_NAME_PREFIX, 'A' + y, x + 1 );
			...
			//main B1 level
			sprintf( szFilename, "%S%c%d_b1.dat", JA25_EXP_MAP_NAME_PREFIX, 'A' + y, x + 1 );
			SetFileManCurrentDirectory( MapsDir );
			hfile = FileOpen( szFilename, FILE_ACCESS_READ | FILE_OPEN_EXISTING, FALSE );
			SetFileManCurrentDirectory( DevInfoDir );
			if( hfile )
			...
			//main B2 level
			sprintf( szFilename, "%S%c%d_b2.dat", JA25_EXP_MAP_NAME_PREFIX, 'A' + y, x + 1 );
			SetFileManCurrentDirectory( MapsDir );
			hfile = FileOpen( szFilename, FILE_ACCESS_READ | FILE_OPEN_EXISTING, FALSE );
			SetFileManCurrentDirectory( DevInfoDir );
			if( hfile )
			...
			//main B3 level
			sprintf( szFilename, "%S%c%d_b3.dat", JA25_EXP_MAP_NAME_PREFIX, 'A' + y, x + 1 );
			SetFileManCurrentDirectory( MapsDir );
			hfile = FileOpen( szFilename, FILE_ACCESS_READ | FILE_OPEN_EXISTING, FALSE );
			SetFileManCurrentDirectory( DevInfoDir );
			if( hfile )
			...
			sprintf( szFilename, "%S%c%d_a.dat", JA25_EXP_MAP_NAME_PREFIX, 'A' + y, x + 1 );
			SetFileManCurrentDirectory( MapsDir );
			hfile = FileOpen( szFilename, FILE_ACCESS_READ | FILE_OPEN_EXISTING, FALSE );
			SetFileManCurrentDirectory( DevInfoDir );
			if( hfile )
			...
			//alternate B1 level
			sprintf( szFilename, "%S%c%d_b1_a.dat", JA25_EXP_MAP_NAME_PREFIX, 'A' + y, x + 1 );
			SetFileManCurrentDirectory( MapsDir );
			hfile = FileOpen( szFilename, FILE_ACCESS_READ | FILE_OPEN_EXISTING, FALSE );
			SetFileManCurrentDirectory( DevInfoDir );
			if( hfile )
			...
			//alternate B2 level
			sprintf( szFilename, "%S%c%d_b2_a.dat", JA25_EXP_MAP_NAME_PREFIX, 'A' + y, x + 1 );
			SetFileManCurrentDirectory( MapsDir );
			hfile = FileOpen( szFilename, FILE_ACCESS_READ | FILE_OPEN_EXISTING, FALSE );
			SetFileManCurrentDirectory( DevInfoDir );
			if( hfile )
			...
			//alternate B3 level
			sprintf( szFilename, "%S%c%d_b3_a.dat", JA25_EXP_MAP_NAME_PREFIX, 'A' + y, x + 1 );
			SetFileManCurrentDirectory( MapsDir );
			hfile = FileOpen( szFilename, FILE_ACCESS_READ | FILE_OPEN_EXISTING, FALSE );
			SetFileManCurrentDirectory( DevInfoDir );
			if( hfile )
...
void WriteSectorSummaryUpdate( UINT8 *puiFilename, UINT8 ubLevel, SUMMARYFILE *pSummaryFileInfo )
{
	...
	if( puiFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] >= 'a' )
		y = puiFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] - 'a';
	else
		y = puiFilename[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] - 'A';
	if( puiFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] < '0' || puiFilename[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] > '9' )
		x = puiFilename[ 1 +JA25_EXP_MAP_NAME_SIZE_OFFSET] - '0' - 1;
	else
		x = (puiFilename[ 1 +JA25_EXP_MAP_NAME_SIZE_OFFSET] - '0') * 10 + puiFilename[ 2 +JA25_EXP_MAP_NAME_SIZE_OFFSET] - '0' - 1;
...
void LoadSummary( UINT8 *pSector, UINT8 ubLevel, FLOAT dMajorMapVersion )
{
	...
	//is bad.
	y = pSector[0+JA25_EXP_MAP_NAME_SIZE_OFFSET] - 'A';
	if( pSector[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] >= '0' && pSector[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] <= '9' )
		x = (pSector[1+JA25_EXP_MAP_NAME_SIZE_OFFSET] - '0') * 10 + pSector[2+JA25_EXP_MAP_NAME_SIZE_OFFSET] - '0' - 1;
	else
		x = pSector[1+JA25_EXP_MAP_NAME_SIZE_OFFSET] - '0' - 1;
...
void ReportError( UINT8 *pSector, UINT8 ubLevel )
{
	...
	mprintf( 10, yp, L"Skipping update for %s. ", str );
	InvalidateScreen();
	yp++;
}
...
void RegenerateSummaryInfoForAllOutdatedMaps()
{
	...
	for( y = 0; y < 16; y++ ) for( x = 0; x < 16; x++ )
	{
		...
		sprintf( str, "%S%c%d", JA25_EXP_MAP_NAME_PREFIX, y + 'A', x + 1 );
		if( gbSectorLevels[x][y] & GROUND_LEVEL_MASK )
		{
...
void SummaryUpdateCallback( GUI_BUTTON *btn, INT32 reason )
{
	if( reason & MSYS_CALLBACK_REASON_LBUTTON_UP )
	{
		...
		sprintf( str, "%S%c%d", JA25_EXP_MAP_NAME_PREFIX, gsSelSectorY + 'A' - 1, gsSelSectorX );
...
void ApologizeOverrideAndForceUpdateEverything()
{
	...
	gfMajorUpdate = TRUE;
	iSummaryButton[ SUMMARY_BACKGROUND ] =
		CreateTextButton( 0, 0, 0, 0, BUTTON_USE_DEFAULT, 0, 0, SCREEN_BUFFER_WIDTH, SCREEN_BUFFER_HEIGHT, BUTTON_TOGGLE, MSYS_PRIORITY_HIGH - 1,
		BUTTON_NO_CALLBACK, BUTTON_NO_CALLBACK );
	SpecifyDisabledButtonStyle( iSummaryButton[ SUMMARY_BACKGROUND ], DISABLED_STYLE_NONE );
	DisableButton( iSummaryButton[ SUMMARY_BACKGROUND ] );
	//Draw it
	...
	for( y = 0; y < 16; y++ ) for( x = 0; x < 16; x++ )
	{
		sprintf( name, "%S%c%d", JA25_EXP_MAP_NAME_PREFIX, y + 'A', x + 1 );
		if( gbSectorLevels[x][y] & GROUND_LEVEL_MASK )
		{
			pSF = gpSectorSummary[x][y][0];
...
INT32					iLoadButton;
INT32					iNewButton;
BOOLEAN				gfRenderStartWindow = FALSE;

void StartLoadMapCallback( GUI_BUTTON *btn, INT32 reason )
{
	if( reason & MSYS_CALLBACK_REASON_LBUTTON_UP )
	{
...
void StartNewMapCallback( GUI_BUTTON *btn, INT32 reason )
{
	if( reason & MSYS_CALLBACK_REASON_LBUTTON_UP )
	{
...
void CreateStartWindow()
{
	...
	iNewButton = CreateTextButton( L"New", FONT12POINT1, FONT_BLACK, FONT_BLACK, BUTTON_USE_DEFAULT,
		20+55, MAP_BOTTOM+45, 50, 26, BUTTON_NO_TOGGLE, MSYS_PRIORITY_HIGH, DEFAULT_MOVE_CALLBACK,
		BtnNewMapCallback );

	gfStartWindowActive = TRUE;

	MSYS_DefineRegion( &MapRegion, MAP_LEFT, MAP_TOP, MAP_RIGHT, MAP_BOTTOM, MSYS_PRIORITY_HIGH, 0,
		MapMoveCallback, MapClickCallback );
	MSYS_DisableRegion( &MapRegion );

	gfRenderStartWindow = TRUE;
}
...
void RenderStartWindow()
{
	...
	if ( gfRenderStartWindow )
	{
		ColorFillVideoSurfaceArea( FRAME_BUFFER, 0, 0, SCREEN_BUFFER_WIDTH, SCREEN_BUFFER_HEIGHT, Get16BPPColor( FROMRGB( 255, 255, 255 ) ) );
		InvalidateScreen( );

		mprintf( 10, 5, L"JA25 EDITOR -- %s Version 1.%02d",
			gszVersionType[ GLOBAL_SUMMARY_STATE ], GLOBAL_SUMMARY_VERSION );
	}
