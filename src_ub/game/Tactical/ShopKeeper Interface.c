...
#define		SKI_TACTICAL_BACKGROUND_START_WIDTH		SCREEN_BUFFER_WIDTH - SKI_TACTICAL_BACKGROUND_START_X
#define		SKI_TACTICAL_BACKGROUND_START_HEIGHT	340

#define		SKI_ITEM_MOVEMENT_AREA_X							85
#define		SKI_ITEM_MOVEMENT_AREA_Y							263
#define		SKI_ITEM_MOVEMENT_AREA_WIDTH					(SCREEN_BUFFER_WIDTH - SKI_ITEM_MOVEMENT_AREA_X)
...
BOOLEAN	gfCanSayMakeTransactionQuote;
...
UINT8		gubDisplayMsgBoxAskingUserToAttachTransmitter=0;
...
*/
	...
	//Blanket the entire screen
	MSYS_DefineRegion( &gSKI_EntireScreenMouseRegions, 0, 0, SCREEN_BUFFER_WIDTH, SCREEN_BUFFER_HEIGHT-40, MSYS_PRIORITY_HIGH-2,
...
//Region to allow the user to drop items to the ground
	MSYS_DefineRegion( &gArmsDealersDropItemToGroundMouseRegions, SKI_DROP_ITEM_TO_GROUND_START_X, SKI_DROP_ITEM_TO_GROUND_START_Y, SCREEN_BUFFER_WIDTH, SCREEN_BUFFER_WIDTH-40, MSYS_PRIORITY_HIGH,
...
//Ja25:  No jake in exp					if ( gbSelectedArmsDealerID != ARMS_DEALER_JAKE )
					{
			...
			if( gbSelectedArmsDealerID == ARMS_DEALER_RAUL )
			{
				...
				if( gArmsDealerStatus[ gbSelectedArmsDealerID ].fHasSoldSomethingToPlayer == FALSE )
				{
					...
					if( gArmsDealersInventory[ ARMS_DEALER_RAUL ][ BARRETT ].ubTotalItems != 0 )
					{
	...
	for( uiCnt=0; uiCnt<SKI_NUM_TRADING_INV_SLOTS; uiCnt++)
	{
		...
		if( ArmsDealerOfferArea[ uiCnt ].fActive )
		{
			...
			if( gbSelectedArmsDealerID == ARMS_DEALER_BETTY )
			{
				...
				if( ArmsDealerOfferArea[ uiCnt ].sItemIndex == TEX_MOVIE_ATTACK_CLYDESDALES ||
						ArmsDealerOfferArea[ uiCnt ].sItemIndex == TEX_MOVIE_WILD_EAST	||
						ArmsDealerOfferArea[ uiCnt ].sItemIndex == TEX_MOVIE_HAVE_HONDA )
				{
			...
			if( PlayersOfferArea[ uiCnt ].uiFlags & ARMS_INV_PLAYERS_ITEM_HAS_VALUE )
			{
				...
				else
				{
					...
					if( gbSelectedArmsDealerID == ARMS_DEALER_RAUL )
					{
						...
						if( PlayersOfferArea[ uiCnt ].sItemIndex == CLIP_CANNON_BALL )
						{
							fAddItemToDealer = FALSE;
						}

						//if the item is the antique musket
						if( PlayersOfferArea[ uiCnt ].sItemIndex == HAND_CANNON )
						{
...
*/
	...
	switch( ubSource )
	{
		...
		case PLAYERS_OFFER_AREA:
			...
			{
				//if the item is the antique musket
				if( gMoveingItem.ItemObject.usItem == HAND_CANNON )
				{
					if( gubQuest[ QUEST_FIND_ANTIQUE_MUSKET_FOR_RAUL ] == QUESTINPROGRESS )
					{
						//if the player hasnt said the quote before
...
//		gusPositionOfSubTitlesX = ( SCREEN_BUFFER_WIDTH - usActualWidth ) / 2 ;
...
*/
	...
	if ( fFirstOne && CanShopkeeperOverrideDialogue( ) )
	{
		...
		if ( sQuoteNum != -1 && gfCanSayMakeTransactionQuote )
		{
		...
		gfCanSayMakeTransactionQuote = TRUE;
	}
}
...
BOOLEAN DoSkiMessageBox( UINT8 ubStyle, INT16 *zString, UINT32 uiExitScreen, UINT8 ubFlags, MSGBOX_CALLBACK ReturnCallback )
{
	SGPRect pCenteringRect= {0, 0, SCREEN_BUFFER_WIDTH, 339 };
...
void InitShopKeeperItemDescBox( OBJECTTYPE *pObject, UINT8 ubPocket, UINT8 ubFromLocation )
{
	...
	switch( ubFromLocation )
	{
		case ARMS_DEALER_INVENTORY:
		{
			...
			if( ( sPosX + 358 ) > SCREEN_BUFFER_WIDTH )
				sPosX = sPosX - ( ( sPosX + 358 ) - SCREEN_BUFFER_WIDTH ) - 5;
...
//	ShadowVideoSurfaceRect( FRAME_BUFFER, 0, 0, SCREEN_BUFFER_WIDTH, SCREEN_BUFFER_HEIGHT );
	...
	if( gfSMDisableForItems )
		DrawHatchOnInventory( FRAME_BUFFER, 0, 0, SCREEN_BUFFER_WIDTH, SCREEN_BUFFER_HEIGHT-178 );
	else
...
void DisplayTheSkiDropItemToGroundString()
{
	...
	usHeight = DisplayWrappedString( SKI_DROP_ITEM_TO_GROUND_START_X, SKI_DROP_ITEM_TO_GROUND_TEXT_START_Y, (UINT16)(SCREEN_BUFFER_WIDTH-SKI_DROP_ITEM_TO_GROUND_START_X), 2, SKI_LABEL_FONT, SKI_TITLE_COLOR, SKI_Text[ SKI_TEXT_DROP_ITEM_TO_GROUND ], FONT_MCOLOR_BLACK, FALSE, CENTER_JUSTIFIED | DONT_DISPLAY_TEXT );

	//display the 'drop item to ground' text
	DisplayWrappedString( SKI_DROP_ITEM_TO_GROUND_START_X, (UINT16)(SKI_DROP_ITEM_TO_GROUND_TEXT_START_Y-usHeight), (UINT16)(SCREEN_BUFFER_WIDTH-SKI_DROP_ITEM_TO_GROUND_START_X), 2, SKI_LABEL_FONT, SKI_TITLE_COLOR, SKI_Text[ SKI_TEXT_DROP_ITEM_TO_GROUND ], FONT_MCOLOR_BLACK, FALSE, CENTER_JUSTIFIED | INVALIDATE_TEXT );
}
