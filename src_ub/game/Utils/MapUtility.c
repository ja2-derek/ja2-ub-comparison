...
BOOLEAN	GenerateRadarMap( INT8 *zFilename, BOOLEAN fSeperateFile, HWFILE pFile )
{
	...
	if ( AddVideoSurface( &vs_desc, &giTempBuffer ) == FALSE )
	{
		return( FALSE );
	}
	// Zero out area!
	ColorFillVideoSurfaceArea( giTempBuffer, 0, 0, (INT16)(SCREEN_BUFFER_WIDTH), (INT16)(SCREEN_BUFFER_HEIGHT), Get16BPPColor( FROMRGB( 0, 0, 0 ) ) );
	...
	RenderOverheadMap( giTempBuffer, 0, (WORLD_COLS/2), 0, 0, SCREEN_BUFFER_WIDTH, 320, TRUE );
  ...
  TrashOverheadMap( );
	...
	pSrcBuf = (UINT16*)LockVideoSurface(giTempBuffer, &uiSrcPitchBYTES);
	...
	UnLockVideoSurface(giTempBuffer);

	// RENDER!
	BltVideoSurface( giTempBuffer, giMiniMap, 0, 20, INTERFACE_START_Y, VS_BLT_FAST | VS_BLT_USECOLORKEY, NULL );


	//QUantize!
	pDataPtr = (UINT8*)LockVideoSurface(gi8BitMiniMap, &uiSrcPitchBYTES);
	pDestBuf = (UINT16*)LockVideoSurface(giTempBuffer, &uiDestPitchBYTES);
	...
	{
		WriteSTIFileEx( pDataPtr, pPalette, MINIMAP_X_SIZE, MINIMAP_Y_SIZE, CONVERT_ETRLE_COMPRESS, 0, pFile );
	}

	UnLockVideoSurface(gi8BitMiniMap);

	// OK, delete thse puppies
	DeleteVideoSurfaceFromIndex( giTempBuffer );
	DeleteVideoSurfaceFromIndex( gi8BitMiniMap );	
