...
BOOLEAN LoadGameSettings()
{
	...
	{
		if( gGameSettings.uiSettingsVersionNumber < 521 )
		{
			gGameSettings.fOptions[ TOPTION_MERC_CASTS_LIGHT ] = TRUE;
		}
		else if( gGameSettings.uiSettingsVersionNumber < 522 )
		{
			gGameSettings.ubSizeOfDisplayCover = 5;
			gGameSettings.ubSizeOfLOS = 5;
		}
		else
		{
			//Initialize the settings
