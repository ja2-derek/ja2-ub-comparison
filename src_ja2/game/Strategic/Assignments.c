...
void CreateDestroyScreenMaskForAssignmentAndContractMenus( void )
{
	...
	{
		MSYS_DefineRegion( &gAssignmentScreenMaskRegion, 0, 0, SCREEN_BUFFER_WIDTH, SCREEN_BUFFER_HEIGHT, MSYS_PRIORITY_HIGHEST - 4 ,
...
void CreateSquadBox( void )
{
 ...
 if( giBoxY + pDimensions.iBottom > SCREEN_BUFFER_HEIGHT-1 )
 {
		pPoint.iY = SquadPosition.iY = SCREEN_BUFFER_HEIGHT-1 - pDimensions.iBottom;
	}
...
void CreateEPCBox( void )
{
  ...
  INT32 iCount;
	 ...
	 if( giBoxY + pDimensions.iBottom > SCREEN_BUFFER_HEIGHT-1 )
	 {
			pPoint.iY = AssignmentPosition.iY = SCREEN_BUFFER_HEIGHT-1 - pDimensions.iBottom;
	 }
...
void SetTacticalPopUpAssignmentBoxXY( void )
{
	...
	if ( ( gsAssignmentBoxesX + 100 ) > SCREEN_BUFFER_WIDTH )
	{
		gsAssignmentBoxesX = SCREEN_BUFFER_WIDTH-100;
	}
...
void CheckAndUpdateTacticalAssignmentPopUpPositions( void )
{
	...
	if( fShowRepairMenu == TRUE )
	{
		...
		{
			gsAssignmentBoxesX = ( INT16 ) ( SCREEN_BUFFER_WIDTH-1 - ( pDimensions2.iRight + pDimensions.iRight ) );
			SetRenderFlags( RENDER_FLAG_FULL );
		}
	...
	else if( fShowAttributeMenu == TRUE )
	{
		...
		{
			gsAssignmentBoxesX = ( INT16 ) ( SCREEN_BUFFER_WIDTH-1 - ( pDimensions2.iRight + pDimensions.iRight + pDimensions3.iRight ) );
			SetRenderFlags( RENDER_FLAG_FULL );
		}
	...
	else
	{
		...
		if( gsAssignmentBoxesX + pDimensions2.iRight  >= SCREEN_BUFFER_WIDTH )
		{
			gsAssignmentBoxesX = ( INT16 ) ( SCREEN_BUFFER_WIDTH-1 - ( pDimensions2.iRight  ) );
			SetRenderFlags( RENDER_FLAG_FULL );
		}
